...
//                        print("response:: \(response)")
    ...
    func createFolderInGdrive(name:String, fileId:String,  completionHandler: @escaping (NSDictionary?, NSError?) -> ()){
        ...
        let values:[String:Any] = [
            "title":"\(name.precomposedStringWithCanonicalMapping)",
...
//                        print("parentFolderId : \(foldrIdStruct.value)")
                        ...
                        if let fileUrl:URL = FileUtil().getFileUrlWithFoldr(fileNm: originalFileName, foldrWholePathNm: files[index].foldrWholePathNm, amdDate: amdDate) {
                            sendToDriveFromLocal(name: originalFileName.precomposedStringWithCanonicalMapping, parentFileId: parentFolderId, fileURL: fileUrl) { responseObject, error in
                                if error == nil {
...
//                return
    ...
    func downloadFolderFromNas(foldrId:Int, foldrWholePathNm:String, userId:String, devUuid:String, deviceName:String, getAccessToken: String, getNewFoldrWholePathNm: String,  getGdriveFolderIdToSave: String, getOldFoldrWholePathNm: String,  getMultiArray: [App.FolderStruct], fileId:String, parent:ContainerViewController){
